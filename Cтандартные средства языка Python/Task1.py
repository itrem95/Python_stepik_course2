...
def create(n):
    """
    Заполняет словарь parents в соответствии с форматом:
    <имя исключения 1> : <имя исключения 2> <имя исключения 3> ... <имя исключения k>
    n: количество считываемых строк ввода
    """
    for _ in range(n):
        path = input().split()
        parents[path[0]] = [] if len(path) == 1 else path[2:]


def check(ex, parent):
    """
    Проверяет, было ли отловлено исключение-предок до исключения ex
    в соответствии с иерархией исключений.
    ex: текущее исключение
    parent:    исключение-предок
    :return:          результат проверки
    """
    if ex == parent:
        return True
    return any([check(child, parent) for child in parents[ex]])


parents = {}
n = int(input())  # количество классов-исключений
create(n)
m = int(input())  # количество проверяемых исключений
used = []

for i in range(m):
    exception = input()
    if any([check(exception, used_one) for used_one in used]):
        print(exception)
